...
const getRecommendationHistory = async (req, res) => {
  try {
    const history = await CareerRecommendation.find({ user: req.user.id })
      .select('inputSnapshot recommendations createdAt')
      .sort({ createdAt: -1 });

    res.status(200).json({ status: 'success', results: history.length, data: { history } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: 'Server error fetching history' });
  }
};
...
const getRecommendationById = async (req, res) => {
  try {
    const result = await CareerRecommendation.findOne({ _id: req.params.id, user: req.user.id });

    if (!result) {
      return res.status(404).json({ status: 'fail', message: 'Recommendation not found' });
    }

    res.status(200).json({ status: 'success', data: { result } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: 'Server error fetching recommendation' });
  }
};

module.exports = { generateRecommendation, getRecommendationHistory, getRecommendationById };