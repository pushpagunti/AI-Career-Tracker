...
const generateRoadmap = async (req, res) => {
  try {
    ...
    const { parsed, rawResponse } = await getRoadmap({
    ...
    // Create progress links for roadmap topics
    ...
    // Create notification after roadmap generation
    await createNotification(
      ...
      `Your learning roadmap for ${targetRole} is ready.`,
...
// includes live progress status per topic
...
const linkTopic = async (req, res) => {
  ...
  try {
    ...
    const linkEntry = roadmap.progressLinks.find(
    ...
    if (!linkEntry) {
      ...
      return res.status(400).json({
        ...
        message: `Topic "${topic}" not found on this roadmap`
    ...
    if (createNew) {
      ...
      const existing = await LearningItem.findOne({
    ...
    res.status(500).json({
      ...
      message: 'Server error linking topic'
